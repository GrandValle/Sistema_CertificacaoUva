...
const garantirNomePadronizado = (nomeOriginal, tipoTela) => {
...
class DocumentoController {
    async salvar(req, res) {
        try {
            ...
            const service = new DocumentoService_1.DocumentoService();
    ...
    async deletarGenerico(req, res, metodoService) {
        try {
            const { id } = req.params;
            if (!id)
                return res.status(400).json({ error: "ID é obrigatório." });
            const service = new DocumentoService_1.DocumentoService();
            const resultado = await service[metodoService](id);
